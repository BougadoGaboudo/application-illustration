...
async function saveFile(file) {
  ...
  // Créer le répertoire s'il n'existe pas (recursive: true)
...
// Récupérer le prix pour un type et une option de background
export async function calculateCommissionPrice(type, hasBackground) {
  try {
    const priceInfo = await prisma.commissionPrice.findUnique({
      where: { type },
    });

    if (!priceInfo) {
      return { error: "Prix non trouvé pour ce type de commission" };
    }

    const price = hasBackground
    ...
    return { error: "Une erreur est survenue lors du calcul du prix" };
  }
}
...
export async function createCommission(formData) {
  ...
  try {
    ...
    const totalAmount = background
      ? commissionPrice.baseAmount + commissionPrice.bgAddon
      : commissionPrice.baseAmount;
...
export async function getCommissionById(id) {
  ...
  try {
    ...
    return {
      error: "Une erreur est survenue lors de la récupération de la commission",
    };
  }
}
...
// UPDATE - Mettre à jour le statut d'une commission et ses tags (admin uniquement)
export async function updateCommissionStatus(id, formData) {
  ...
  try {
    ...
    if (tagsInput !== undefined) {
      // Supprimer les tags existants pour cette commission
      await prisma.commissionTag.deleteMany({
        where: { commissionId },
      });

      // Si des tags ont été fournis, les ajouter
      if (tagsInput.trim()) {
        ...
        // Créer ou récupérer chaque tag et l'associer à la commission
        for (const tagName of tagNames) {
          // Rechercher ou créer le tag
          ...
          // Associer le tag à la commission
          await prisma.commissionTag.create({
            data: {
    ...
    // Récupérer la commission mise à jour avec ses tags
    ...
    return { success: true, commission: finalCommission };
  } catch (error) {
...
export async function updateCommission(id, formData) {
  ...
  try {
    ...
    if (
      ...
      return {
        error:
          "Vous ne pouvez modifier votre commission que lorsqu'elle est en attente",
      };
    }
    ...
    // Si le type change, mettre à jour le prix
    let commissionPriceId = commission.commissionPriceId;

    if (type !== commission.type) {
      const newPrice = await prisma.commissionPrice.findUnique({
        where: { type },
      });
...
export async function deleteCommission(id) {
  ...
  try {
    ...
    return {
      error: "Une erreur est survenue lors de la suppression de la commission",
    };
  }
}
...
export async function uploadSketch(id, formData) {
  ...
  try {
    ...
    if (commission.status !== "in_progress") {
      return {
        error:
          "Vous ne pouvez envoyer un croquis que pour une commission en cours",
      };
    }
...
export async function uploadFinalIllustration(id, formData) {
  ...
  try {
    ...
    if (commission.status !== "completed") {
      return {
        error:
          "Vous ne pouvez envoyer l'illustration finale que pour une commission terminée",
      };
    }
    ...
    return {
      error: "Une erreur est survenue lors de l'envoi de l'illustration finale",
    };
  }
}
